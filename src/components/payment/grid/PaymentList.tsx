...
export const PaymentList = () => {
  ...
  const { data, isLoading, refetch } = useGetAllPayments(query);
  ...
  const columns: ColumnsType<IPayment> = [
    ...
    },
    {
      ...
      key: 'paymentName',
      filterIcon: getFilterValue('paymentName') ? <CheckCircle /> : <Search />,
      filterDropdown: (
        <TableFilterSearch
          onSearch={(value) => updateFilter('paymentName', value)}
    ...
    },
    {
      ...
      render: (record) => (
        <div className='flex flex-row gap-1'>
          <ActionButton.Edit href={`/admin/payment/create/${record.id}`} />
          <ActionButton.Edit
            tooltip='modal edit'
  ...
  return (
    <div className='min-h-screen bg-background-100 px-4 py-6 dark:bg-background-dark-100 sm:px-6 lg:px-8'>
      ...
      >
        <CreatePaymentForm form={form} onFinish={handleSubmit} />
      </Drawer>
    </div>
  );
};
