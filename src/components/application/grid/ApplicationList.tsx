...
export const ApplicationList = () => {
  ...
  const { data, isLoading, refetch } = useGetAllApplications(query);
  ...
  const handleSubmit = async (values: IApplication) => {
    try {
      if (values.id) {
  ...
  const onEdit = (application: IApplication) => {
    ...
    },
    {
      ...
      render: (record) => (
        <div className='flex flex-row gap-1'>
          <ActionButton.Edit href={`/admin/application/create/${record.id}`} />
          <ActionButton.Edit
            tooltip='modal edit'
  ...
  return (
    <div className='min-h-screen bg-background-100 px-4 py-6 dark:bg-background-dark-100 sm:px-6 lg:px-8'>
      ...
      >
        <CreateApplicationForm form={form} onFinish={handleSubmit} />
      </Drawer>
    </div>
  );
};
